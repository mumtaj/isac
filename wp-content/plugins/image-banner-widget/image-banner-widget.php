...
define('SHAILAN_IBW_VERSION','1.4');
...
if(!class_exists('shailan_BannerWidget')){
	class shailan_BannerWidget extends WP_Widget {
		...
		function form($instance) {
			...
			<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:'); ?> <?php $this->help_link('title'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label></p>
			...
			<p><label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Display for:'); ?>
				<select name="<?php echo $this->get_field_name('category'); ?>" id="<?php echo $this->get_field_id('category'); ?>" > 	
					...
					<option value="shailan-home-only" <?php if('shailan-home-only' == $category){ echo "selected=\"selected\""; } ?> >Homepage only</option>		
					 ...
					 /* wp_dropdown_pages( array( 'name' => 'page_for_posts', 'echo' => 0, 'show_option_none' => __( '&mdash; Select &mdash;' ), 'option_none_value' => '0', 'selected' => get_option( 'page_for_posts' ) ) )*/ 
