...
<?php
/* Template Name: view-group-members  */
get_header();
$gid = (!empty($_GET['view_members'])) ? base64_decode(trim($_GET['view_members'])) : '';
?>
<SCRIPT>   
          ...
          <table width="450" border="0" cellspacing="0" cellpadding="0"  align="center">
            ...
            <tr>
              <td height="35" align="center"><span style="color:#F00; font-size:18px; text-align:center"><strong>ACCESS DENIED. <br />
                ...
                <br />
                <div class="btn_cent"><a href="registration-option"><img src="<?php bloginfo('template_url'); ?>/images/btn_register.jpg" border="0"  /></a>&nbsp; &nbsp; <a href="student-login"><img src="<?php bloginfo('template_url'); ?>/images/btn_login.jpg" border="0"  /></a></div></td>
            </tr>
          </table>
        </div>
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
elseif($_SESSION['SESS_MEMBER_ID']!='')
{
...
//$reg_details = mysql_query("SELECT * FROM registration WHERE group_id_isac like '%$gid%'");
$reg_details = mysql_query("SELECT * FROM application WHERE group_id like '%$gid%'");
if(@mysql_num_rows($reg_details) < 1) die('<br><br><br>&nbsp; Sorry! No matching records found. Go back to <a href="my-isac" title="My ISAC">My ISAC</a>');
  ...
  <div class="art-content-layout-row">
    <div class="art-layout-cell art-content"> 
      ...
      <div class="summary_form reg top_spacing">
        <div class="rht_tab">
         <?php if(!empty($_GET['gp']) && $_GET['gp'] =='y') { ?>
         <ul>
         ...
         <?php } else { ?>
          <ul>
            ...
            <li><a href="my-isac"><img src="<?php bloginfo('template_url'); ?>/images/back_to_ISAC_button.jpg" class="btn_apply_stu" title="BACK TO MYISAC" border="0" hspace="1"></a></li>
		...
		{
			?>
        <h2 style="color:#900; text-align:center"> <b> Create group member failed!<br />
          Please check email. </b> </h2>
        <?php
		}
		...
		{
			?>
        <h2 style="color:#900; text-align:center"> <b> Successfully deleted. </b> </h2>
        <?php
		}
		else if ($_GET['del'] == 'false')
		{
			?>
        <h2 style="color:#900; text-align:center"> <b> Delete record failed! </b> </h2>
        <?php
		}
	?>
        <br />
	...
	$app_sql = "SELECT * FROM groups WHERE group_id = '$gid'";
	...
	$gpname = $app_row['group_name'];
	//
	$query_owner_gp = "SELECT * FROM application WHERE group_id = '$gid'"; 
...
?>
		...
		?></td>
		  ...
		  //////////        Starting of third drop downlist /////////
			...
			?></td>
              ...
              <td width="178"><input type="text" value="<?=$email;?>" name="owner_email" class='grad1_hack' readonly="readonly" />
                <input type="hidden" value="owner_program" name="owner_program" class='grad1'/>
                <input type="hidden" name="gender" value="<?=$gender;?>" /></td>
		...
		?>
                ...
                <input type="hidden" value="<?php echo $value_set ?>" name="value_set" /></td>
            </tr>
            <tr>
	...
	if(mysql_num_rows($result_owner_gp) == 0)
	{
		...
		?>
        <div style=" font-size:14px; padding:10px 20px; text-align:center"> You will first need to select a program for your group. After this, you can add individuals to your group for the same program.
       ...
       <!-- <h2 class="fonts_h2_headlines">Your details:</h2>
        <table width="900" border="0" cellspacing="0" cellpadding="0" class="admin_tbl">
          ...
          <tr>
            ...
            <td width="150"><form action="<?php echo SERVER_URL;?>student_panel_dashboard_group_edit" method="post" onsubmit="return confirm('Are you sure you want to edit this record?')">
            ...
            <td width="150" class="brn_n"><form action="<?php bloginfo('template_url'); ?>/isac_process/delete_to_group_process.php" method="post" onsubmit="return confirm('Are you sure you want to delete this record?')">
                <input type="hidden" value="<?php echo $row_owner_gp['formid'];?>" name="formid" />
...
$query_display_gp = "SELECT * FROM application WHERE group_id = '$gid'  ORDER BY id desc";
$result_display_gp = mysql_query($query_display_gp);
		...
		{
	?>
        <h2 class="fonts_h2_headlines">Group Name: <?=ucfirst($gpname);?></h2>
<?php
$gp_data_q = "SELECT * FROM application WHERE group_id = '$gid' LIMIT 1";
$gp_data_rs = mysql_query($gp_data_q);
$gp_data = @mysql_fetch_array($gp_data_rs);

?>        
Program Name : <?=$gp_data['program'];?><br />
Program Duration : <?php echo $gp_data['duration'].' '.'Weeks' ?><br />
Leader Name : <?=ucfirst($gp_data['firstname']).' '.ucfirst($gp_data['lastname']);?>
...
(In USD)</th>
          ...
          <?php 
			...
			$formid_stu = $row_display_gp['formid'];
		$reg_fee_stauts = $row_display_gp['registration_fee'];
		$firstinstallement_status = $row_display_gp['fee_first_installment'];        
		$secondinstallement_status = $row_display_gp['fee_second_installment'];
		$due_installement_first = '300';
		$due_reg = '45';
		$final_payment = $row_display_gp['second_installment'];
		...
		$cls = ($row_display_gp['isacid'] == $isacid)? " class='bld' " : '';
			?>
          <tr>
            
            <td   <?=$cls;?>><?php echo $row_display_gp['arrival']; 
			
			?>
            
            </td>
            <td  <?=$cls;?>><?php echo $row_display_gp['firstname']; ?></td>
            <td  <?=$cls;?>><?php echo $row_display_gp['lastname']; ?></td>
            <td  <?=$cls;?>><?php echo $row_display_gp['email']; ?></td>
            <td  <?=$cls;?>>$<?php echo $row_display_gp['fee']; ?></td>
            <td  <?=$cls;?>>
            <?php echo $total_payment;   ?>
            </td>
            <td  <?=$cls;?>><?php echo $total_due_next_installement;   ?></td>
          </tr>
          <?php
	  }
...
?></td>
              ...
              <td ><select name="gender" class="grad1">
...
?>
	...
	$query_display_gp01 = "SELECT * FROM application WHERE group_id = '$gid' ORDER BY id desc";
	$result_display_gp01 = mysql_query($query_display_gp01);
	if(mysql_num_rows($result_display_gp01) == 0)
	{
		
		?>
        <div style=" font-size:14px; padding:10px 20px; text-align:center"> You do not have any members in your group. Please add members by entering individual details above and clicking ‘Submit’.
