...
/* Template Name: flight_details_partner_edit  */
...
while($member_details = mysql_fetch_array($sql))
{
	$partner_member_isacid=$member_details['isacid'];
}
$details = mysql_query("SELECT * FROM registration WHERE isacid = '$partner_member_isacid'");
...
<div class="art-content-layout">
 ...
 <div class="left_sect reg lft">
  <div style="text-align:right;padding-top:5px">
    <h4 style="margin:5px 0" >Student name: <span style="font-weight:normal !important"><?php echo $firstname;   ?></span></h4>
 ...
 <?php // checking for flight details
			$sql_details = "SELECT * FROM arrival_flight WHERE formid='$formid'";
			$result_details = mysql_query($sql_details);
			while($row_details = mysql_fetch_array($result_details))
			{
			$formid_flight = $row_details['formid'];
			$airline_name = $row_details['airline_name'];
			$flight_num = $row_details['flight_num'];
			$d = $row_details['arrival_day'];
			$m = $row_details['arrival_month'];
			$y = $row_details['arrival_year'];
			$arrival_time = $row_details['arrival_time'];
			$ampm = $row_details['arrival_time1'];
			$arrival_from = $row_details['commnets'];
			$arrival_info = $row_details['arrival_info'];
			}
			
 ?>
	<img src="<?php bloginfo('template_url'); ?>/images/spacer.gif" width="1" height="8" />
    <form action="<?php bloginfo('template_url'); ?>/isac_process/flight_action_edit.php" method="post" name="applicationform1" id="appfrm" >
      <table width="520" border="0" cellspacing="2" cellpadding="2" class="top_spacing">
       <tr>
          <td class="form_text text_aln_rt">Arriving From<br /></td>
          <td class="space_for_form_2"><input type="text" name="arrivingfrom" value="<?php echo $arrival_from;  ?>" /></td>
        </tr>
         <tr>
          <td class="form_text text_aln_rt">Terminal no. </td>
          <td class="space_for_form_2"><input type="text" name="arrival_details" value="<?php echo $arrival_from;  ?>" /></td>
        </tr>
        <tr>
          <td width="174" class="form_text text_aln_rt">Flight name</td>
          <td width="285" class="space_for_form"><input  type="text" name="name" value="<?php echo $airline_name;  ?>"/></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td class="space_for_form_2"><input  type="text" name="fno" value="<?php echo $flight_num;  ?>"/></td>
        </tr>
        <tr>
          <td colspan="3"><img src="<?php bloginfo('template_url'); ?>/images/spacer.gif" width="1" height="3" /></td>
        </tr>
        ...
        <tr>
          ...
          <td class="space_for_form_2"><select name="time" id="time" class="dropdown_small">
              <option value="<?php echo $arrival_time;  ?>"  selected="selected"><?php echo $arrival_time;  ?></option>
...
frmvalidator.addValidation("arrivingfrom","req","Please fill the arriving from field");
