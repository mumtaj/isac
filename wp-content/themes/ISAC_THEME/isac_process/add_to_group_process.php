...
if ($_SESSION['SESS_MEMBER_ID'] != '' AND $_SESSION['SESS_MEMBER_GROUP_TYPE'] == 'GROUP')
{
	...
	if($owner_program != '')
	{
		...
		$duration = $_POST['owner_subcat']; //$_POST['value_set'];
	...
	if($correct_owner_email == 'correct' OR $correct_owner_email == '')
	{
		if(mysql_num_rows($check_result) == 0)
		{
			...
			if(empty($program)) $program = $_POST['program'];
			if(empty($fee))  $fee = $_POST['cost_fees'];
			if(empty($arrival)) $arrival = $_POST['arrival'];
			if(empty($duration)) $duration = $_POST['duration'];
			...
			while($row_arrival_d = mysql_fetch_array($query_arrival_division))
			{
	 		...
	 		$qry_application = "INSERT INTO application(isacid,isac_groupname,firstname,lastname,gender, email,program,user_type, 
	  							program_id,group_id,formid,fee, arrival,duration,formdate, first_installment, second_installment, 
								arrival_month,arrival_year, group_limit) 
								VALUES('$isacid_member', '$gp_name', '$name','$lastname','$msex','$email','$program', '$gp', 
								'$program_temp','$group_process',
								'$formid','$fee','$arrival','$duration', '$dateofregistration', '$firstinstall','$secondinstall', 
								'$arrival_month','$arrival_year', '$grouplimit')";
						
			$result = @mysql_query($qry_application);
		
		
			  $qry_registration = "INSERT INTO registration(isacid,firstname,lastname,gender,email,password, dateofregistration, 
			  					group_isac,group_id_isac) 
							VALUES('$isacid_member','$name','$lastname','$msex','$email','".md5($pass)."','$dateofregistration','$gp',' 
								$group_process')";
		...
		else if (mysql_num_rows($check_groupmember_result) == 0)
		{
			...
			while($row_arrival_d = mysql_fetch_array($query_arrival_division))
			{
			 ...
			 $qry_application = "INSERT INTO application(isacid,isac_groupname,firstname,lastname,email, program,user_type, 
									program_id,group_id,formid,fee, arrival,duration,formdate, first_installment, second_installment, 
									arrival_month,arrival_year, group_limit) 
								VALUES('$isacid_member', '$gp_name', '$name','$lastname','$email',  '$program', '$gp', 
									'$program_temp','$group_process',
									'$formid','$fee','$arrival','$duration', '$dateofregistration', '$firstinstall','$secondinstall', 
									'$arrival_month','$arrival_year', '$grouplimit')"; 
