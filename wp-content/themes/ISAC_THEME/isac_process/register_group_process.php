...
Please retain these details in order to login to your account, apply for program and check your application status, We thank you for registering on our site and look forward to welcome you to our programs.";
...
$qry = "INSERT INTO registration(isacid,firstname,lastname,email,password,date,month,year,gender,dateofregistration,phone_number,phone_mobile,skype,address,zip,city,state,country,group_isac,group_id_isac) VALUES('$isacid','$firstname','$lastname','$email','".md5($password)."','$datebirth','$month','$year','$gender','$dateofregistration','$phone_home','$cell','$skype','$address','$zip','$city','$state','$country','$gp','$gp_isacid')";
	...
	if($result) {
		if(!empty($_COOKIE['prefill'])) unset($_COOKIE['prefill']);
		else setcookie('prefill','$email',time()+60*60*24*10,'/', 'indiastudyabroad.org',false,true); 	
		$_COOKIE['prefill'] = $email;
