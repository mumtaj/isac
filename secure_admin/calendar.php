...
if( (!empty($_POST['doSearch'])) && $_POST['doSearch']==1) 
{
    //query database
...
<?php
    $quer2=mysqlquery("SELECT  DISTINCT * FROM isac_arrival_table GROUP BY arrival_date_full ORDER BY arrival_date_full ASC"); 
    //////////        Starting of first drop downlist /////////
    echo "<select name='arrival' class='grad1'>";
    echo "<option selected='selected' value='00000'>--SELECT--</option>";
    while($prog_arrival = mysqli_fetch_array($quer2)) 
    { 
...
if ( ! empty($_POST['doSearch']) && $_POST['doSearch'] )  //a search request was made display my search results 
{
    $qry11_na = "SELECT * FROM category ";
    
    if ( !empty($program_id) && $program_id != "00000" ) 
        $qry11_na .= "WHERE category LIKE '%$program_id%'";

    //echo $qry11_na ;

    $sql11_na = mysqlquery($qry11_na);

    while($result_na_pr = mysqli_fetch_array($sql11_na))
    {
        //p($result_na_pr);
...
?>
   <table width="1160" border="0" cellspacing="0" cellpadding="0" class="admin_tbl">
     ...
     <?php 
$quer_display=mysqlquery($query_all);

  while($prog_display = mysqli_fetch_array($quer_display)) {  ?>
...
<table width="600" border="0" cellspacing="0" cellpadding="0" class="admin_tbl">
  ...
  <tr>
	...
	$total_stu = mysqli_fetch_array(mysqlquery($query_all_stu));
  ...
  <tr>
    <td>MALE:</td>
    <td class="brn_n"><?php $total_stu_m = mysqli_fetch_array(mysqlquery($query_all_m));
  ...
  <tr>
    <td>FEMALE:</td>
    <td class="brn_n"><?php $total_stu_fm = mysqli_fetch_array(mysqlquery($query_all_fm));
