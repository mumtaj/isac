...
// How many adjacent pages should be shown on each side?
	...
	$fee_data = array();
	$fee_sql = "SELECT group_id, registration_fee_count, fee_first_inst_count FROM group_members_count ORDER BY group_id DESC";
	...
	$i=0;
	$registration_group_data = array();
	$download_group_data = array();
	foreach ($group_limit_data as $limit_data)
	{
		if ($limit_data['group_limit_group_id'] == $fee_data[$i]['fee_group_id'] AND $limit_data['group_members_count'] == $fee_data[$i]['reg_fee_count'] AND $limit_data['group_members_count'] != $fee_data[$i]['fee_first_inst_count'])
		{
			array_push($registration_group_data, '"'.$limit_data['group_limit_group_id'].'"');
			array_push($download_group_data, $limit_data['group_limit_group_id']);
		}
		$i++;
	}
	
	$implode_registration_data = implode(',',$registration_group_data); //Final data implode
	$implode_download_group_data = implode(',',$download_group_data); //download_group_data implode
	
	$total_pages = count($registration_group_data); // Total Records
	
	/* Setup vars for query. */
	$targetpage = "admin_dashboard-registered-applications-group.php"; 	//your file name  (the name of this file)
	...
	//Get Print DATA
	$query = "SELECT * FROM $tbl_name WHERE group_id IN ($implode_registration_data) LIMIT $start, $limit ";
...
<img src="images/theme_register.jpg" width="990" height="350" alt="Programs" title="Programs" /></div --> 
  ...
  <li>|</li>
  <li><a href="admin_dashboard-registered-applications-group.php" title="Registered Applications"><strong>Registered Applications</strong></a></li>
  <li>|</li>
  ...
  <div class="total_count">
    ...
    <!--<div style="padding:10px" >
    	<form action="download_groups.php" method="post">
            ...
            <input type="hidden" value="register" name="group_type" />
