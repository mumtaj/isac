...
// How many adjacent pages should be shown on each side?
	...
	$i=0;
	$deposit_group_data = array();
	$download_group_data = array();
	foreach ($group_limit_data as $limit_data)
	{
		//if ($limit_data['group_limit_group_id'] == $fee_data[$i]['fee_group_id'] AND $limit_data['group_limit_count'] <= $fee_data[$i]['reg_fee_count'] AND $limit_data['group_limit_count'] <= $fee_data[$i]['fee_first_inst_count'] AND $limit_data['group_limit_count'] != $fee_data[$i]['fee_second_inst_count'])
		...
		{
			array_push($deposit_group_data, '"'.$limit_data['group_limit_group_id'].'"');
	...
	$implode_deposit_data = implode(',',$deposit_group_data); //Final data implode
	$implode_download_group_data = implode(',',$download_group_data); //download_group_data implode
	
	$total_pages = count($deposit_group_data); // Total Records
	
	/* Setup vars for query. */
	$targetpage = "admin_dashboard-deposits-recived-applications-group.php"; 	//your file name  (the name of this file)
	...
	//Get Print DATA
	$query = "SELECT * FROM $tbl_name WHERE group_id IN ($implode_deposit_data) LIMIT $start, $limit ";
...
<img src="images/theme_register.jpg" width="990" height="350" alt="Programs" title="Programs" /></div --> 
 ...
 <?php
  ...
  <li>|</li>
  <li><a href="admin_dashboard-deposits-recived-applications-group.php" title="Deposits Received"><strong>Deposits Received</strong></a></li>
  <li>|</li>
  ...
  <div class="total_count">
    ...
    <!--<div style="padding:10px" >
    	<form action="download_groups.php" method="post">
            ...
            <input type="hidden" value="deposit" name="group_type" />
