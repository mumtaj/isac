...
<?php
	...
	$group_limit_result = mysql_query($group_limit_sql);
	
	while ($group_limit_row = mysql_fetch_array($group_limit_result))
	{
	...
	$fee_data = array();
	$fee_sql = "SELECT group_id, registration_fee_count FROM group_members_count ORDER BY group_id DESC";
	$fee_result = mysql_query($fee_sql);
	
	while ($fee_row = mysql_fetch_array($fee_result))
	{
		array_push($fee_data, array('fee_group_id'=>$fee_row['group_id'], 
												'fee_count'=>$fee_row['registration_fee_count']));
	}
	
	//Final Data Puching to Array
	$i=0;
	$incomplete_group_data = array();
	$download_group_data = array();
	foreach ($group_limit_data as $limit_data)
	{
		
		/*print $limit_data['group_limit_group_id']." == ".$fee_data[$i]['fee_group_id']."<br>";
		 print $limit_data['group_limit_count'] ."!=". $fee_data[$i]['fee_count']."<br /><br /><br />";*/
		
		
		if ($limit_data['group_limit_group_id'] == $fee_data[$i]['fee_group_id'] AND $limit_data['group_members_count'] != $fee_data[$i]['fee_count'])
		{
			array_push($incomplete_group_data, '"'.$limit_data['group_limit_group_id'].'"');
	...
	$implode_incomplete_data = implode(',',$incomplete_group_data); //Final data implode
	$implode_download_group_data = implode(',',$download_group_data); //download_group_data implode
	
	$total_pages = count($incomplete_group_data); // Total Records
	
	/* Setup vars for query. */
	$targetpage = "admin_dashboard-incomplete-applications-group.php"; 	//your file name  (the name of this file)
	...
	//Get Print DATA
	$query = "SELECT * FROM $tbl_name WHERE group_id IN ($implode_incomplete_data) LIMIT $start, $limit ";
 ...
 <?php
  ...
  <li>|</li>
  	<?php
		...
		?>
  <li><a href="admin_dashboard-incomplete-applications-group.php" title="Incomplete Applications"><strong>Incomplete Applications</strong></a></li>
  <li>|</li>
  ...
  <div class="total_count">
    ...
    <!--<div style="padding:10px" >
    	<form action="download_groups.php" method="post">
            ...
            <input type="hidden" value="incomplete" name="group_type" />
